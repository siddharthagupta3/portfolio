...
export default function Hero() {
  ...
  useEffect(() => {
    const updateNavigatePosition = () => {
      ...
      const btnBottomY = btnRect.bottom - containerRect.top; // place under the button

      // Also ensure StackHeading sits below the header (Siddhartha). Use header bottom if available.
      const header = document.querySelector('header');
      let headerBottomY = 0;
      if (header) {
        const headerRect = header.getBoundingClientRect();
        headerBottomY = headerRect.bottom - containerRect.top;
      }

      // final Y is the greater of button bottom or header bottom + small offset
      const finalY = Math.max(btnBottomY, headerBottomY + 8);

      setNavigateX(Math.round(centerX));
      setNavigateY(Math.round(finalY));
    };
  ...
  return (
    <section
      id="hero"
      className="relative bg-black text-white overflow-x-hidden flex flex-col items-center pt-[calc(var(--header-height,4.5rem)+0.75rem)] pb-0 max-md:pb-2 md:pt-[calc(var(--header-height,5rem)+1rem)] md:pb-6"
    >
      {/* Background glow */}
      ...
      <div className="md:hidden container w-full flex justify-center px-12 mb-12 relative z-20 shrink-0">
        <StackHeading />
      </div>

      {/* Desktop: position StackHeading under the Navigate button (calculates horizontal center) */}
      <div className="hidden md:block absolute left-0 right-0 z-20 pointer-events-none" style={{ top: 'calc(var(--header-height,5rem) + 3rem)' }}>
      ...
      <div
        ...
        style={{ transform: dimensions.width > 0 && dimensions.width < 768 ? 'translateY(86px)' : 'translateY(86px)' }}
      >
        {/* Connections */}
      ...
      {/* Footer */}
      <div className="relative z-20 flex items-center justify-center gap-4 text-gray-500 tracking-[0.2em] text-[10px] md:text-xs select-none mt-12 max-md:mt-10 max-md:mb-3 max-md:w-full max-md:px-4 md:mt-20 md:mb-0 shrink-0 whitespace-nowrap">
